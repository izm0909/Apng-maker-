...
- [ ] 必要なライブラリのインストール (`@imgly/background-removal`, `upng-js` 等)
- [ ] 基本的なディレクトリ構造とCSS変数の設定 (デザインシステム)
...
### Phase 3: コア機能実装 (背景透過)
- [ ] `@imgly/background-removal` を用いた背景削除処理の実装
- [ ] 透過画像のCanvasへの描画

### Phase 4: コア機能実装 (アニメーション & APNG)
- [ ] Canvas上でのフレームアニメーション描画ロジックの実装
- [ ] 複数フレームをキャプチャし、`upng-js` でAPNGバイナリに変換する処理
- [ ] ダウンロード機能の実装

### Phase 5: UI/UX改善 & 最適化
- [ ] ローディング表示、エラーハンドリング
- [ ] デザインのブラッシュアップ (Glassmorphism, Animations)
- [ ] LINEスタンプ仕様への適合チェック（サイズ、容量警告など）
